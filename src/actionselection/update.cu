...
__global__ void updatePotential_MSN_D1 ( int nt, neuron_t *n_MSN_D1 ){
    ...
    if ( i < N_MSN_D1 ){
        ...
        n_MSN_D1 -> v[ i ] += DT * ( - ( n_MSN_D1 -> v[ i ] - Er_MSN_D1 ) + Vc_MSN_D1 + ( rho * n_MSN_D1 -> ig[ i ] ) ) / TAU_MSN_D1;
...
__global__ void updatePotential_MSN_D2 ( int nt, neuron_t *n_MSN_D2 ){
    ...
    if ( i < N_MSN_D2 ){
        ...
        n_MSN_D2 -> v[ i ] += DT * ( - ( n_MSN_D2 -> v[ i ] - Er_MSN_D2 ) + Vc_MSN_D2 + ( rho * n_MSN_D2 -> ig[ i ] ) ) / TAU_MSN_D2;
...
__global__ void updatePotential_FSI ( int nt, neuron_t *n_FSI ){
    ...
    if ( i < N_FSI ){
        ...
        n_FSI -> v[ i ] += DT * ( - ( n_FSI -> v[ i ] - Er_FSI ) + Vc_FSI + ( rho * n_FSI -> ig[ i ] ) ) / TAU_FSI;
...
__global__ void updatePotential_STN ( int nt, neuron_t *n_STN ){
    ...
    if ( i < N_STN ){
        ...
        n_STN -> v[ i ] += DT * ( - ( n_STN -> v[ i ] - Er_STN ) + Vc_STN + ( rho * n_STN -> ig[ i ] ) ) / TAU_STN;
...
__global__ void updatePotential_GPe ( int nt, neuron_t *n_GPe ){
    ...
    if ( i < N_GPe ){
        ...
        n_GPe -> v[ i ] += DT * ( - ( n_GPe -> v[ i ] - Er_GPe ) + Vc_GPe + ( rho * n_GPe -> ig[ i ] ) ) / TAU_GPe;
...
__global__ void updatePotential_GPi ( int nt, neuron_t *n_GPi ){
    ...
    if ( i < N_GPi ){
        ...
        n_GPi -> v[ i ] += DT * ( - ( n_GPi -> v[ i ] - Er_GPi ) + Vc_GPi + ( rho * n_GPi -> ig[ i ] ) ) / TAU_GPi;
...
__global__ void updatePotential_SNc ( int nt, neuron_t *n_SNc ){
    ...
    if ( i < N_SNc ){
        ...
        n_SNc -> v[ i ] += DT * ( - ( n_SNc -> v[ i ] - Er_SNc ) + Vc_SNc + ( rho * n_SNc -> ig[ i ] ) ) / TAU_SNc;
...
__global__ void updatePotential_PTN ( int nt, neuron_t *n_PTN ){
    ...
    if ( i < N_PTN ){
        ...
        n_PTN -> v[ i ] += DT * ( - ( n_PTN -> v[ i ] - Er_PTN ) + Vc_PTN + ( rho * n_PTN -> ig[ i ] ) ) / TAU_PTN;
...
__global__ void updatePotential_PTI ( int nt, neuron_t *n_PTI ){
    ...
    if ( i < N_PTI ){
        ...
        n_PTI -> v[ i ] += DT * ( - ( n_PTI -> v[ i ] - Er_PTI ) + Vc_PTI + ( rho * n_PTI -> ig[ i ] ) ) / TAU_PTI;
...
__global__ void updatePotential_Th ( int nt, neuron_t *n_Th ){
    ...
    if ( i < N_Th ){
        ...
        n_Th -> v[ i ] += DT * ( - ( n_Th -> v[ i ] - Er_Th ) + Vc_Th + ( rho * n_Th -> ig[ i ] ) ) / TAU_Th;
...
void change_input_neuron ( int nt, neuron_t *n_PSN, neuron_t *n_CMPf ){
    ...
    for ( ; i < per_chanel_N_PSN * 10; i++ ){
        ...
        double r = sfmt_genrand_real2 ( &( n_PSN -> rng ) );
        n_PSN -> s[ i ] =  r < ( PHI_MIN_PSN + ( ( ( PHI_MAX_PSN - PHI_MIN_PSN ) * chanel10 ) / N_change ) * n_PSN -> select[ i ] );
        n_PSN -> ts[ i ] = ( n_PSN -> s[ i ] ) * ( nt + 1 ) + ( !( n_PSN -> s[ i ] ) ) * ( n_PSN -> ts[ i ] );
    }
    // 選択肢11
    for ( ; i < per_chanel_N_PSN * 11; i++ ){
        ...
        double r = sfmt_genrand_real2 ( &( n_PSN -> rng ) );
        n_PSN -> s[ i ] =  r < ( PHI_MIN_PSN + ( ( ( PHI_MAX_PSN - PHI_MIN_PSN ) * chanel11 ) / N_change ) * n_PSN -> select[ i ] );
        n_PSN -> ts[ i ] = ( n_PSN -> s[ i ] ) * ( nt + 1 ) + ( !( n_PSN -> s[ i ] ) ) * ( n_PSN -> ts[ i ] );
    }
    // 選択肢12
